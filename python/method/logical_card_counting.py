...
class LogicalCardCounting(BaseBaccaratGambler):
    ...
    def start(self):
        ...
        msg = "下小注"
        if abs(logical_value) > BIG_SPEC:
            msg = "下大注"

        if logical_value < -PLAY_SPEC:
            self.decision = '下閒'
            
        elif logical_value > PLAY_SPEC:
            self.decision = '下莊'
        else:
            msg = ""
            self.decision = '觀察一把'
        self.appendix_message = msg
    @except_decorator
    def _set_init_attribute(self):
        ...
        self.middle_card_num = self.total_deck * 2#一副牌有每四個花色, 四張牌
        print("若遇洗牌, 請重啟程式")
